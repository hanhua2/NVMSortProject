...
#pragma omp for
        for(int i=start;i<start+elenum;i++){
            int digit=determineDigitBucket(kth_byte,(*(begin_itr+i)).key);
            //cout << "kth_byte: " << kth_byte << " digit: " << digit << endl;
            localHists[th][digit]++;
        }
#pragma omp barrier
#pragma omp for
            ...
            for(int ii=0;ii<processes;ii++){
                ...
                for(int i=0;i<kRadixBin;i++){
                    ...
                    if(pID<var_p-1){
                        ...
                        ph[pID][i]=part*pID+gh[i];
                        pt[pID][i]=part*(pID+1LL)+gh[i]+res; // extra added to last process
                    }
                }
                ...
                for(int i=0;i<kRadixBin;i++){
                    ...
                    while(head<pt[pID][i]){
                        ...
                        while(k!=i&&ph[pID][k]<pt[pID][k]){
                            ...
                            k=determineDigitBucket(kth_byte,v.key);
                        }
                        if(k==i){
...
#pragma omp single // allocate buckets to processes for repair
            {
                SumCi=0;
                long long pfpN=kRadixBin/var_p; // 256/var_p
                    ...
                    long long tail=gt[i];
                    {
                        for(int pID=0;pID<processes;pID++){
                            ...
                            while(head<pt[pID][i]&&head<tail){
                                auto v=*(begin_itr+head);head++;
                                if(determineDigitBucket(kth_byte,v.key)!=i){
                                    while(head<=tail){
                                        tail--;
                                        auto w=*(begin_itr+tail);
                                        if(determineDigitBucket(kth_byte,w.key)==i){
                                            *(begin_itr+(head-1))=w;
                                            *(begin_itr+tail)=v;
                                            break;
                                        }
...
#pragma omp task
                    ...
                    //std::sort((begin_itr+starts[i])->key ,begin_itr+(starts[i]+cnt[i]));
                }
            }
#pragma omp taskwait
        }
...
template <class RandomIt>
inline void insert_sort_core_count(RandomIt s, RandomIt e, long long &count)
{
    for (RandomIt i = s + 1; i < e; ++i) {
        if (compare(*i, *(i - 1))) {
            RandomIt j;
...
template<int kth_byte,class RandomIt>
inline void PARADIS_core_count(RandomIt s,RandomIt t,RandomIt begin_itr, long long &count, long long &r_count, int processes=1){
    long long cnt[MaxKisuu]={0};
    long long elenum=distance(s,t);
    long long start=distance(begin_itr,s);
    if (kth_byte >5) {
        cout << "kth_byte: " <<kth_byte << endl;
...
#pragma omp for // repair
            for(int k=0;k<processes;k++){
                for(long long i=pfp[k];i<pfp[k+1];i++){
                    cout << "i: " << i << " gh[i]: " << gh[i] << " gt[i]: " << gt[i] << endl;
                    long long tail=gt[i];
                    // {
                    //     for(int pID=0;pID<processes;pID++){
                    //         long long head=ph[pID][i];
...
#pragma omp task
                    ...
                    PARADIS_core_count<(kth_byte > 0 ? (kth_byte - 1) : 0)>(begin_itr+starts[i],begin_itr+(starts[i]+cnt[i]),begin_itr,count,r_count,std::max(nextStageThreads,1));
                }else if(cnt[i]>1){
                    //insert_sort_core_count(begin_itr+starts[i],begin_itr+(starts[i]+cnt[i]), count);
                    insert_sort_core_(begin_itr+starts[i],begin_itr+(starts[i]+cnt[i]));
                    //std::sort((begin_itr+starts[i])->key ,begin_itr+(starts[i]+cnt[i]));
                }
            }
...
template<class RandomIt>
inline void PARADISCount(RandomIt s,RandomIt t,int threadNum, long long &count, long long &r_count){
    const size_t vsize=3;
    //const size_t vsize = sizeof(s->key);
    cout << "vsize: " << vsize << endl;
    omp_set_nested(1);
    PARADIS_core_count<vsize-1>(s,t,s, count, r_count, threadNum);
}
